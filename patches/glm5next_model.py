...
class Glm5NextModel(nn.Module):
    ...
    @staticmethod
    def _remap_hf_layout_names(weights):
        """Remap newer HF-transformers glm5_next checkpoint naming (used by the
        local-inference-lab NVFP4 export) onto the flat zai-org layout this
        port expects: attn_hc.X -> hc_attn_X, ffn_hc.X -> hc_ffn_X,
        self_attn.forget_gate.* -> self_attn.*, and the fused KDA
        self_attn.conv1d.weight -> q/k/v_conv1d.weight (verified [q;k;v] order,
        bit-identical slices vs the zai-org checkpoint). No-op for checkpoints
        already in the original layout."""
        for args in weights:
            name, tensor = args[:2]
            rest = args[2:]
            if ".attn_hc." in name:
                yield (name.replace(".attn_hc.", ".hc_attn_"), tensor, *rest)
            elif ".ffn_hc." in name:
                yield (name.replace(".ffn_hc.", ".hc_ffn_"), tensor, *rest)
            elif ".self_attn.forget_gate." in name:
                yield (name.replace(".forget_gate.", "."), tensor, *rest)
            elif name.endswith(".self_attn.conv1d.weight"):
                n = tensor.shape[0] // 3
                for i, proj in enumerate(("q_conv1d", "k_conv1d", "v_conv1d")):
                    yield (
                        name.replace(".conv1d.", f".{proj}."),
                        tensor[i * n : (i + 1) * n],
                        *rest,
                    )
            else:
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        weights = self._remap_hf_layout_names(weights)
