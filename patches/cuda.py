...
class CudaPlatformBase(Platform):
    ...
    def _get_indexer_block_alignment(cls, vllm_config: VllmConfig) -> int | None:
        ...
        #
        # SM120 (GLM-5.3-Flash overlay): DeepGEMM's fp8 paged-MQA kernel only
        # supports block_kv == 64 on arch 12 (csrc/apis/attention.hpp), so the
        # storage block must tile at 64, not 32. Align block_size to
        # index_kpool * max(PAGED_MQA_PAGE_SIZES) (= 256 for kpool 4) so
        # compressed_kernel_block_size() lands on 64 (e.g. mamba-aligned 1664
        # -> 1792, storage 448 = 7 * 64). min-alignment (128 -> 1664, storage
        # 416) tiles at 32 and trips the SM120 assert.
        if cls.has_device_capability(120):
            return index_kpool * max(PAGED_MQA_PAGE_SIZES)
