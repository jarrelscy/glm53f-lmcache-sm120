...
class Glm5NextMTP(nn.Module, DeepseekV2MixtureOfExperts):
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        for name, loaded_weight in weights:
            ...
            # Mixed-precision modelopt exports (local-inference-lab NVFP4) ship
            # the MTP layer's routed experts as MXFP8 (fp8_e4m3 weight + per-32
            # e8m0 uint8 weight_scale, dynamic activations). The draft module is
            # built with BF16 experts (the quant-config targets name
            # "model.language_model.layers.45.mlp.experts", which never matches
            # the draft's rewritten "mtp_block." prefix), so dequantize the pair
            # to BF16 here and fall through to the normal expert loading path.
            if ".mtp_block.mlp.experts." in name and (
                (
                ...
                entry = _pending_mxfp8.setdefault(base, {})
                entry[
                    "scale" if name.endswith(".weight_scale") else "weight"
                ] = loaded_weight
                if "weight" not in entry or "scale" not in entry:
                    continue
                w_fp8, e8m0 = entry["weight"], entry["scale"]
                del _pending_mxfp8[base]
                group = w_fp8.shape[1] // e8m0.shape[1]
                loaded_weight = (
                    w_fp8.to(torch.float32)
                    * torch.exp2(e8m0.to(torch.float32) - 127.0)
