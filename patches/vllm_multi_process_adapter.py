    ...
    def get_finished(
        ...
        for request_id, (r_future, r_block_ids) in self.retrieve_futures.items():
            ...
            if not r_result:
                ...
                # OVERLAY FIX: a failed retrieve leaves this request's blocks
                # UNWRITTEN (the server transfers object groups sequentially
                # and breaks on the first failure), yet vLLM would resume as
                # if all promised tokens were restored -- silent state
                # corruption. Mirror the unhealthy-branch handling: flag the
                # blocks in error_block_ids so vLLM recomputes them.
                self.error_block_ids.update(r_block_ids)
