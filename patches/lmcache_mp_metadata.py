...
from lmcache.utils import init_logger as _init_logger

_STORE_PROBE_LOG = _init_logger(__name__)
...
class LMCacheMPRequestMetadata:
    ...
    def GetStoreMetadata(
        ...
        sw_excluded_groups: "frozenset[int]" = frozenset(),
        ...
        Args:
            ...
            sw_excluded_groups: engine group ids that are REAL sliding-window
                attention (vLLM ``SlidingWindowSpec``), e.g. glm5_next's DSA
                indexer ``tail_cache`` (block_size 4). Unlike align-mode Mamba
                -- which physically allocates one block per sequence chunk and
                so scales with length -- a true SWA group caps its block count
                at ``ceil(window / block_size)`` regardless of sequence length
                (here 1 block = 4 tokens). Including it in the coverage ``min``
                below collapses the storable prefix to the window (the observed
                ``allocated_tokens=4`` no-store bug), and its single physical
                window block cannot be sliced by token range without cross-chunk
                mis-attribution. These groups are therefore EXCLUDED from the
                prefix bound and their KV is NOT transferred (block_ids zeroed);
                the tiny window is recomputed from the restored latent on load,
                exactly as the DSA indexer ``k_cache`` (compression-skipped) is.
        """
        ...
        # 16-token full-attention ID). Real sliding-window groups
        # (``sw_excluded_groups``) are skipped: their block count is capped at
        # the window, not the sequence, so they would floor the min (see arg).
        allocated_lengths = tracker.num_allocated_blocks()
        bound_groups = [
            engine_group_idx
            for engine_group_idx in range(num_engine_groups)
            if engine_group_idx not in sw_excluded_groups
        ]
        allocated_tokens = (
            min(
                allocated_lengths.get(engine_group_idx, 0)
                * group_tokens_per_block[engine_group_idx]
                for engine_group_idx in bound_groups
            )
            if bound_groups
            else 0
        )
        ...
        _STORE_PROBE_LOG.warning(
            "[store-probe] req=%s len_tokens=%d allocated_tokens=%d "
            "computed_tokens=%d num_scheduled=%d vllm_hit=%d lmcache_hit=%d "
            "num_stored=%d alloc_blocks=%s min_avail=%d staging=%d chunks=%d",
            tracker.request_id,
            len(tracker.all_token_ids),
            allocated_tokens,
            computed_tokens,
        ...
        if num_chunks >= 1:
            ...
            # Do not transfer real sliding-window groups (see arg): their single
            # window block cannot be attributed to a token-range chunk, so store
            # nothing and let the window recompute from the restored latent.
            for engine_group_idx in sw_excluded_groups:
                if 0 <= engine_group_idx < len(block_ids):
                    block_ids[engine_group_idx] = []
            _STORE_PROBE_LOG.warning(
                "[grp-probe] STORE req=%s start=%d end=%d per_group_blocks=%s",
                tracker.request_id,
                start_token_idx,
                end_token_idx,
                [len(block_ids[g]) for g in range(len(block_ids))],
            )
    ...
    def GetRetrieveMetadata(
        ...
        Args:
            ...
            group_tokens_per_block: per-engine-group tokens covered by one
                ...
                KV cache spec ``block_size``. Must each divide
                ``lmcache_tokens_per_chunk`` (hybrid models can mix different values).
            sw_excluded_groups: real sliding-window engine group ids whose KV
                was never stored (see :meth:`GetStoreMetadata`); their block_ids
                are zeroed here so retrieve loads nothing for them.
        """
        ...
        if end_token_idx > start_token_idx:
            ...
            # Real sliding-window groups were never stored; load nothing.
            for engine_group_idx in sw_excluded_groups:
                if 0 <= engine_group_idx < len(block_ids):
                    block_ids[engine_group_idx] = []
            _STORE_PROBE_LOG.warning(
                "[grp-probe] RETRIEVE req=%s start=%d end=%d per_group_blocks=%s",
                tracker.request_id,
                start_token_idx,
                end_token_idx,
                [len(block_ids[g]) for g in range(len(block_ids))],
