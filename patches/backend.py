...
class MLAAttentionImpl(AttentionImplBase[T], Generic[T]):
    ...
    def do_kv_cache_update(
        ...
        k_pe = k_pe.squeeze(1)
        # GLM-5.3-Flash NoPE MLA overlay: the model has qk_rope_head_dim==0, so
        # k_pe arrives with a 0-width last dim. The packed ``fp8_ds_mla`` cache
        # layout is fixed at 512 latent + 64 rope tail and the C++ kernel asserts
        # pe_dim==64. Synthesize a zero 64-wide rope tail: zeros in the cache rope
        # slot contribute nothing to QK^T (the query tail is zero-padded to match
        # in forward_mqa), so this is bit-for-bit equivalent to NoPE while
        # satisfying the kernel's layout requirement. Gated to fp8_ds_mla + 0 pe.
        if kv_cache_dtype == "fp8_ds_mla" and k_pe.shape[-1] < 64:
            pad = 64 - k_pe.shape[-1]
            k_pe = torch.nn.functional.pad(k_pe, (0, pad))
