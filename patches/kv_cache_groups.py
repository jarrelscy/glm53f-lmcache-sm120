...
def create_engine_group_infos_from_vllm(
    ...
    if vllm_groups:
        ...
        for engine_group_id, group in enumerate(vllm_groups):
            ...
            for name in group.layer_names:
                # EXCLUDE real sliding-window layers (e.g. glm5_next's DSA
                # indexer ``tail_cache``, block_size 4) from all engine groups,
                # exactly as the compression-skipped indexer ``k_cache`` is
                # (it is absent from every vLLM group's ``layer_names``). Their
                # KV is deliberately not transferred -- the store path zeroes
                # their block IDs and the tiny window recomputes from the
                # restored latent on load. If they instead formed a server-side
                # kernel group, the server would expect ``num_chunks *
                # blocks_per_chunk`` (e.g. 448 IDs per 1792-token chunk) block
                # IDs that the store omits and would skip the WHOLE store
                # ("STORE block ID underflow"). Align-mode Mamba/linear layers
                # are NOT matched by ``_is_sliding_window_spec`` and stay grouped
                # (they physically scale with the sequence).
                layer_spec = (
                    per_layer_specs[name]
                    if isinstance(per_layer_specs, Mapping) and name in per_layer_specs
                    else group_spec
                )
                if _is_sliding_window_spec(layer_spec):
                    continue
