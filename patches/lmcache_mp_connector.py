...
class LMCacheMPConnector(KVConnectorBase_V1, SupportsHMA):
    ...
    def __init__(
        ...
        # Engine groups that are REAL sliding-window attention (vLLM
        # ``SlidingWindowSpec``), e.g. glm5_next's DSA indexer ``tail_cache``
        # (block_size 4). A true SWA group caps its allocated block count at the
        # window (1 block here) regardless of sequence length, so it floors the
        # storable-prefix ``min`` in GetStoreMetadata and collapses every store
        # to the window (the observed ``allocated_tokens=4`` no-store bug); its
        # single window block also cannot be sliced by token range without
        # cross-chunk mis-attribution. Detected via ``_is_sliding_window_spec``
        # -- which does NOT match align-mode Mamba/linear layers (those scale
        # with the sequence and belong in the min) -- so only genuine SWA groups
        # are excluded from the bound and skipped from transfer.
        from lmcache.integration.vllm.kv_cache_groups import (
            _is_sliding_window_spec,
        )

        sw_excluded: set[int] = set()
        for engine_group_idx, group in enumerate(vllm_groups):
            spec = group.kv_cache_spec
            inner = getattr(spec, "kv_cache_specs", None)
            leaves = list(inner.values()) if isinstance(inner, dict) else [spec]
            if leaves and all(_is_sliding_window_spec(leaf) for leaf in leaves):
                sw_excluded.add(engine_group_idx)
        self._sw_excluded_groups: frozenset[int] = frozenset(sw_excluded)
        if self._sw_excluded_groups:
            logger.info(
                "LMCacheMPConnector: excluding real sliding-window engine "
                "groups %s from the store bound and KV transfer (window "
                "recomputed on load)",
                sorted(self._sw_excluded_groups),
            )
    ...
    def _process_retrieve_requests(
        ...
        for request_tracker in self.request_trackers.values():
            ...
            r_metadata = LMCacheMPRequestMetadata.GetRetrieveMetadata(
                ...
                sw_excluded_groups=self._sw_excluded_groups,
            )
